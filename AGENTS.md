...
- Application bootstrap and locale initialization: `oh_my_ruyi/app.py` and `oh_my_ruyi/bootstrap.py`.
...
- First-use detection and setup step/status UI: `oh_my_ruyi/first_use.py`; the
  ...
  operations.
- Mutable provisioning selections, FSM, and invalidation: `oh_my_ruyi/state.py`,
  `oh_my_ruyi/state_machine.py`, `oh_my_ruyi/core/fsm.py`, and `oh_my_ruyi/core/models.py`.
- Qt-free ruyi provisioning boundary: `oh_my_ruyi/ruyi_facade.py` and `oh_my_ruyi/infra/ruyi_adapter.py`.
- QThread workers, flashing interception, cancellation, and privileged helpers:
  `oh_my_ruyi/workers.py` and `oh_my_ruyi/worker_manager.py`.
- Repository model and mutations: `oh_my_ruyi/repo_manager.py` and `oh_my_ruyi/infra/repo_manager.py`.
...
- Release discovery, downloads, activation, PATH state, and telemetry:
  `oh_my_ruyi/version_manager.py` and `oh_my_ruyi/controllers/version_controller.py`.
- Disk discovery, mount topology, and target fingerprints:
  `oh_my_ruyi/host_storage.py` and `oh_my_ruyi/infra/os_storage.py`.
